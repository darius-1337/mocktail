...
const MAPPINGS: readonly (readonly [new (...args: never[]) => Error, Mapping])[] = [
  [
    UnknownKindError,
    {
      status: 404,
  ...
  [
    ...
    {
      status: 400,
      body: (e) => ({ error: 'invalid parameter', ...(e as InvalidParamError).problem }),
    },
  ],
  [
    InvalidSeedError,
...
export function toErrorResponse(c: Context, error: unknown) {
  ...
  for (const [type, mapping] of MAPPINGS) {
    if (error instanceof type) return c.json(mapping.body(error), mapping.status);
  }
  return null;
}