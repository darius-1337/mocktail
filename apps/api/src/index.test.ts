...
describe("GET /v1/gen/:kind", () => {
	...
	it.each(["nope", "SIMPLE", ""])(
		"returns 400 for invalid band %s",
		async (band) => {
...
describe("cross-layer: served data honours the contract", () => {
	it.each([...registry.keys()])(
		"everything served for %s validates",
		async (kind) => {
			const body = await json(
				await get(`/v1/gen/${kind}?count=50&band=hostile`),
			);
			const validator = registry.get(kind);

			for (const value of body.data) {
				expect(validator?.validate(value).valid, `${kind}: ${value}`).toBe(
					true,
				);
			}
		},
...
describe("POST /v1/validate/:kind", () => {
	...
	it("rejects and explains a bad value", async () => {
		const body = await json(
			await post("/v1/validate/es-dni", { value: "12345678A" }),
		);
...
it("seeded responses are cacheable but not immutable", async () => {
	const header =
		(await get("/v1/gen/es-dni/abc")).headers.get("Cache-Control") ?? "";
	expect(header).toContain("max-age=");
	expect(header).not.toContain("immutable");
});

it("random-seed responses are never cached", async () => {
	expect((await get("/v1/gen/es-dni")).headers.get("Cache-Control")).toBe(
		"no-store",
	);
});
