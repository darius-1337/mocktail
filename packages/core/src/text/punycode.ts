...
function adaptNumber(delta: number, numPoints: number, firstTime: boolean): number  {
...
function decodeLabel(input: string): string {
    ...
    for(let index = start; index < input.length; ) {
        ...
        for(let w = 1, constant = BASE; ; constant += BASE) {
            ...
            if(digit > Math.floor((MAX_INT - i) / w)) throw new PunycodeError('overflow');

            i += digit * w;
            const time = constant <= bias ? TMIN : constant >= bias + TMAX ? TMAX : constant - bias;
            if(digit < time) break;

            if(w > Math.floor(MAX_INT / (BASE - time))) throw new PunycodeError('overflow');
            w *= BASE - time;
        }
        ...
        if(Math.floor(i / out) > MAX_INT - num) throw new PunycodeError('overflow');
