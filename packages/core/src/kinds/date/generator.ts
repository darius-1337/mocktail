...
const STRATEGIES: readonly [Strategy, ...Strategy[]] = [
    ...
    { rarity: 0.45, build: (r, lo, hi) => snapMonthToEdge(lo + r.int(hi - lo + 1), r) },
    { rarity: 0.75, build: (r, lo, hi) => leapDayWithin(lo + r.int(hi - lo + 1), lo, hi) },
    {
    rarity: 0.95,
    build: (r, lo, hi) =>
...
export function generateDate(rng: Rng, opts: GeneratedOptions): string {
    ...
    const low = fromParam === null ? toEpochDay(1950, 1, 1) : toEpochDay(...fromParam);
