...
function checkLocal(local: string): string | null {
	...
	if (local.length > MAX_LOCAL)
		return `the maximum characters allowed for local part is ${MAX_LOCAL}`;
	...
	if (local.startsWith('"') && local.endsWith('"') && local.length >= 2) {
		...
		while (i < inner.length) {
			...
			if (c === "\\") {
				if (i + 1 >= inner.length)
					return "dangling escape in quoted local part";
			...
			if (cp < 32 || cp === 127)
				return "control character in quoted local part";
			i++;
		}
...
function checkDomain(domain: string): string | null {
	...
	for (const label of domain.split(".")) {
		if (label.length === 0 || label.length > 63)
			return "invalid domain label length";
		if (!/^[A-Za-z0-9-]+$/.test(label))
			return "invalid character in domain label";
		if (label.startsWith("-") || label.endsWith("-"))
			return "domain label edge hyphen";
	}
	return null;
}
...
export function validateEmail(input: string): ValidationResult {
	...
	if (input.length > MAX_TOTAL)
		return { valid: false, reason: `exceeds ${MAX_TOTAL} characters` };
