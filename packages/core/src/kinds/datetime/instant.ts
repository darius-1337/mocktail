...
import type { KindParam } from '../../contracts.js';
...
const STRATEGIES: readonly [Strategy, ...Strategy[]] = [
  ...
  { rarity: 0.97, offset: (r) => r.pick([840, -720, 345]), time: () => 0, extreme: true },
];
...
export const RANGE_PARAMS: readonly KindParam[] = [
  { name: 'from', description: 'earliest date, inclusive yyyy-mm-dd', default: DEFAULT_FROM, maxLength: 10, pattern: /^\d{4}-\d{2}-\d{2}$/ },
  { name: 'to',   description: 'latest date, inclusive yyyy-mm-dd', default: DEFAULT_TO,   maxLength: 10, pattern: /^\d{4}-\d{2}-\d{2}$/ },
];