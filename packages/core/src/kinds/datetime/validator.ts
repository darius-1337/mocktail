...
const SHAPE =
  ...
  export function validateDateTime(input: string): ValidationResult {
    ...
    if(match === null) {
        return { valid: false, reason: 'expected ISO 8601 with offset ex: [2026-08-15T22:57:00Z]' }
    }
    ...
    if(offset !== 'Z') {
        ...
        const offsetMinutes = Number(offset.slice(4, 6));
            ...
            if (offsetMinutes > 59) return { valid: false, reason: 'invalid offset minutes' };
            if (offsetHours > MAX_OFFSET_HOURS) return { valid: false, reason: 'offset beyond ±14:00' };
