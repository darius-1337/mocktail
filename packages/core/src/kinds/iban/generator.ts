...
function withSpanishCheckDigits(bban: string): string {
  ...
  return bank + branch + cccDigit(`00${bank}${branch}`) + cccDigit(account) + account;
}
...
function withDutchCheckDigit(bban: string): string {
  ...
  for (let attempt = 0; attempt < 11; attempt++) {
    ...
    account = account.slice(0, 8) + String((Number(account[8]) + 1) % 10) + account[9];
  }
...
function withFrenchRibKey(bban: string): string {
  ...
  const key =
    97 - ((89 * digitsOnly(bank) + 15 * digitsOnly(branch) + 3 * digitsOnly(numeric)) % 97);
