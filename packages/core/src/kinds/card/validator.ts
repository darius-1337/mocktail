...
export const BRANDS = {
	visa: {
		label: "Visa",
		prefixes: ["424242", "400000", "411111"],
		lengths: [16, 19],
	},
	mastercard: {
		label: "Mastercard",
		prefixes: ["555555", "510510", "222300"],
		lengths: [16],
	},
	amex: {
		label: "American Express",
		prefixes: ["378282", "371449"],
		lengths: [15],
	},
	discover: {
		label: "Discover",
		prefixes: ["601111", "644000"],
		lengths: [16, 19],
	},
	jcb: { label: "JCB", prefixes: ["356600"], lengths: [16, 19] },
	diners: {
		label: "Diners Club",
		prefixes: ["305693", "360000"],
		lengths: [14, 16],
	},
} as const;
...
export function validateCard(input: string): ValidationResult {
	...
	if (!/^\d+$/.test(value)) {
		return {
			valid: false,
			reason: "must contain only digits, spaces or dashes",
		};
	}
...
export function validateBrandedCard(
	brand: Brand,
	input: string,
): ValidationResult {
	const value = normalise(input);
	...
	if (!(spec.lengths as readonly number[]).includes(value.length)) {
		return {
			valid: false,
			reason: `invalid ${spec.label}, expected: ${spec.lengths.join(" or ")}, got ${value.length}`,
		};
	}
