...
import { PopulateError, populate } from "./populate.js";
...
describe("populate", () => {
	...
	it("adding a field does not change existing ones", () => {
		...
		const after = populate({
			...req,
			fields: { ...req.fields, phone: "es-dni" },
		});
...
it("rejects too many fields", () => {
	...
	expect(() => populate({ seed: "s", count: 1, fields })).toThrow(
		PopulateError,
	);
});
...
it("rejects count × fields above the cell limit", () => {
	...
	expect(() => populate({ seed: "s", count: 10_000, fields })).toThrow(
		PopulateError,
	);
});
